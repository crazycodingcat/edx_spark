...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.mapValues(lambda x: tokenize(x))
googleRecToToken = googleSmall.mapValues(lambda x: tokenize(x))
...
def tf(tokens):
    ...
    for t in tokens:
        tfs[t] += 1
    return dict((k, float(v)/len(tokens)) for (k, v) in tfs.iteritems())
...
def idfs(corpus):
    ...
    #map them to a pair RDD for counting
    tokenCountPairTuple = uniqueTokens.map(lambda t: (t, 1.0))
    #count number of documents containing each unique token by reduce
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(lambda a, b: a + b)
    return (tokenSumPairTuple.mapValues(lambda x: N/x))
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)
    tfIdfDict = dict((k, v * idfs[k]) for (k, v) in tfs.iteritems())
    return tfIdfDict
...
def norm(a):
    ...
    """
    return sum([a[k] * a[k] for k in a.iterkeys()]) ** 0.5
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda record: ("%s %s" % (record[1], record[0]), record[2]))

trueDupsRDD = (sims
               .join(goldStandard)
               .map(lambda x: float(x[1][0])))
trueDupsCount = trueDupsRDD.count()
avgSimDups = trueDupsRDD.reduce(lambda a, b: a + b) / trueDupsCount

nonDupsRDD = (sims
              .subtractByKey(goldStandard)
              .map(lambda x: float(x[1])))
avgSimNon = nonDupsRDD.reduce(lambda a, b: a + b) / nonDupsRDD.count()
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.mapValues(lambda x: tokenize(x))
googleFullRecToToken = google.mapValues(lambda x: tokenize(x))
...
amazonWeightsRDD = amazonFullRecToToken.mapValues(lambda x: tfidf(x, idfsFullBroadcast.value))
googleWeightsRDD = googleFullRecToToken.mapValues(lambda x: tfidf(x, idfsFullBroadcast.value))
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.mapValues(lambda x: norm(x)).collectAsMap()
amazonNormsBroadcast = sc.broadcast(amazonNorms)
googleNorms = googleWeightsRDD.mapValues(lambda x: norm(x)).collectAsMap()

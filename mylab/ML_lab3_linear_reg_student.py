...
def parsePoint(line):
    ...
    """
    splits = line.split(',')
    return LabeledPoint(splits[0], splits[1:])
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return (labelsAndPreds.map(lambda (l, p): squaredError(l, p))
                   .mean()) ** 0.5
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        gradient = trainData.map(lambda lp: gradientSummand(w, lp)).reduce(lambda a, b: a + b)
...
# TODO: Replace <FILL IN> with appropriate code
firstModel = LinearRegressionWithSGD.train(parsedTrainData, numIters, alpha, miniBatchFrac, 
...
def twoWayInteractions(lp):
    ...
    """
    twoWayFeatures = map(lambda (a, b): a * b, itertools.product(lp.features, lp.features))
    return LabeledPoint(lp.label, np.hstack((lp.features, twoWayFeatures)))
